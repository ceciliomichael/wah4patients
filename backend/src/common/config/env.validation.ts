...
export const envValidationSchema = Joi.object({
  ...
  CORS_ALLOWED_ORIGINS: Joi.string().trim().min(1).optional(),
  FRONTEND_ORIGIN: Joi.string().trim().min(1).optional(),
})
  .or('CORS_ALLOWED_ORIGINS', 'FRONTEND_ORIGIN')
  .keys({
    BACKEND_API_KEY: Joi.string().trim().min(24).required(),
    BACKEND_REQUEST_LOGGING_ENABLED: Joi.boolean()
      .truthy('true')
      .falsy('false')
      .default(false),
    ...
    MFA_CHALLENGE_TOKEN_TTL_SECONDS: Joi.number()
      ...
      .max(600)
      .default(180),
    SECURITY_VERIFICATION_TOKEN_SECRET: Joi.string().trim().min(32).required(),
    SECURITY_VERIFICATION_TOKEN_TTL_SECONDS: Joi.number()
      .integer()
      .min(60)
      .max(1200)
      .default(300),
    ...
    MPIN_LOCK_DURATION_MINUTES: Joi.number()
      .integer()
      .min(1)
      .max(60)
      .default(15),
