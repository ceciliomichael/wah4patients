...
- Webhook Endpoints
You must implement three webhook endpoints that the gateway will call
- Practitioner Directory Endpoint
Expose a secured endpoint (for example /api/fhir/practitioners) that returns practitioner {code, display, active} records
...
Before exchanging data, you must register your organization with the system administrator. This process establishes your identity in the network.

### Registration Process

**Note:** Provider registration is currently an administrative process. Please contact the system administrator to register your organization.

You will need to provide the following information:

- Organization Name
- Provider Type (e.g., hospital, clinic)
- Base URL (publicly accessible webhook endpoint)
- Practitioner List Endpoint (for example `/api/fhir/practitioners`)

Once registered, you will receive:

- **Provider ID:** Your unique identifier (UUID)
- **API Key:** Secret key for authenticating your requests
- **Gateway Auth Key:** Validate this in incoming gateway calls

2
## Implement Webhook + Practitioner Sync Endpoints

The gateway communicates with your system through callbacks and practitioner-directory sync. Implement your webhook endpoints plus a practitioner list endpoint.

webhooks.mmd

#### Webhook Interaction Pattern

In addition to the three webhook endpoints below, expose `/api/fhir/practitioners` (or your configured `practitionerListEndpoint`) and call `POST /api/v1/providers/{id}/practitioners/webhook` after practitioner updates so gateway cache stays current.
...
2. 2.
Handle webhook statuses as currently implemented by gateway: `SUCCESS` and `ERROR`
3. 3.
...
Called when another provider pushes unsolicited data to you

POST`{your_base_url}/fhir/receive-push`

Incoming Push from Gateway
...
2. 2.
Store the received unsolicited data
3. 3.
Respond with `200 OK` to confirm receipt
...
POST`https://gateway.wah4pc.com/api/v1/fhir/request/Patient`
...
For all resource-specific request payloads, see [Request Formats](/docs/request-formats).
...
Send resources directly to another provider without a prior request. Useful for sending referrals, appointments, or unsolicited results.

### Initiate a Push

POST`https://gateway.wah4pc.com/api/v1/fhir/push/Appointment`
...
**Immediate Delivery:** Unlike queries, push requests are delivered immediately. If the target provider accepts the data (returns 200 OK), the transaction is marked as COMPLETED instantly.
...
The gateway prevents identical requests (same requester, target, and lookup inputs) within 5 minutes. Handle 429 errors gracefully and avoid immediate retries.
